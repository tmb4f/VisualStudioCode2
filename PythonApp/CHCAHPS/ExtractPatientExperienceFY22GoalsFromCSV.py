...
dfs = df.iloc[:,[1,2,4,5,6,7,8,9,10,11,12]]
...
service_line_columns = ['Service_Line','Epic_Department_Id','Question','FY2022_Unit_Score','Organization','FY2022_Organization_Score','Service','FY2022_Service_Score','Clinical_Area','FY2022_Clinical_Area_Score','FY2022_All_SL_Score']
...
for i,row in dfs.iterrows():
	...
	if row[2] in Service_Line_Dictionary.keys():
		...
		for index, item in enumerate(topbox_li):        
			...
			if index == 2: Question = topbox_li[index]
			if index == 3: FY2022_Unit_Score = topbox_li[index]
			if index == 4: Organization = topbox_li[index]
			if index == 5: FY2022_Organization_Score = topbox_li[index]
			if index == 6: Service = topbox_li[index]
			if index == 7: FY2022_Service_Score = topbox_li[index]
			if index == 8: Clinical_Area = topbox_li[index]
			if index == 9: FY2022_Clinical_Area_Score = topbox_li[index]
			if index == 10: FY2022_All_SL_Score = topbox_li[index]
		...
		dfo = dfo.append(dict(zip(service_line_columns, (Service_Line,Epic_Department_Id,Question,FY2022_Unit_Score,Organization,FY2022_Organization_Score,Service,FY2022_Service_Score,Clinical_Area,FY2022_Clinical_Area_Score,FY2022_All_SL_Score))),ignore_index=True)
...
dfo_unpivot_1 = pd.melt(dfo.replace('null',np.nan),
   ...
   id_vars=['Service_Line','Epic_Department_Id','Question','Organization','FY2022_Organization_Score','Service','FY2022_Service_Score','Clinical_Area','FY2022_Clinical_Area_Score','FY2022_All_SL_Score'],
   ...
   value_vars=dfo.columns.drop(['Service_Line','Epic_Department_Id','Question','Organization','FY2022_Organization_Score','Service','FY2022_Service_Score','Clinical_Area','FY2022_Clinical_Area_Score','FY2022_All_SL_Score']).tolist(),
   value_name='Unit_Score',
   var_name='FY') \
   .dropna() \
   .sort_values(['Service_Line','Epic_Department_Id','Question','Organization','FY2022_Organization_Score','Service','FY2022_Service_Score','Clinical_Area','FY2022_Clinical_Area_Score','FY2022_All_SL_Score'])
...
dfo_unpivot_2 = pd.melt(dfo_unpivot_1.replace('null',np.nan),
   id_vars=['Service_Line','Epic_Department_Id','Question','Organization','Organization_Score','Service','Service_Score','Clinical_Area','Clinical_Area_Score','Unit_Score'],
   value_vars=dfo_unpivot_1.columns.drop(['Service_Line','Epic_Department_Id','Question','Organization','Organization_Score','Service','Service_Score','Clinical_Area','Clinical_Area_Score','Unit_Score']).tolist(),
   value_name='All_SL_Score',
   var_name='FY') \
   .dropna() \
   .sort_values(['Service_Line','Epic_Department_Id','Question','Organization','Organization_Score','Service','Service_Score','Clinical_Area','Clinical_Area_Score','Unit_Score'])
