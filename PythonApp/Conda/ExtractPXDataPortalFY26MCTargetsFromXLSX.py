...
# fnamein = "Tom's Copy-FY25 Goals_07052024.xlsx"
# fnameout = "Tom's Copy-FY25 Goals_07052024 Staging.txt"
# fnamein = "Tom's Copy-FY25 Goals 7242024_07242024.xlsx"
# fnameout = "Tom's Copy-FY25 Goals 7242024_07242024 Staging.txt"
# fnamein = "Tom's Copy-MC CH FY25 Goals 7292024_07292024.xlsx"
# fnameout = "Tom's Copy-MC CH FY25 Goals 7292024_07292024 Staging.txt"
fnamein = "Tom's Copy-FY26 MC Goals 06242025_06242025.xlsx"
fnameout = "Tom's Copy-FY26 MC Goals 06242025_06242025 Staging.txt"
...
for sheet in Sheet_names_list :
    ...
    if len(df_to_print.columns) > 4: # Number of columns in sheet
        ...
        for i,row in df_to_print.iterrows(): # For each row in sheet
            ...
            if row_index > -1:
                # dfo = dfo.append(dict(zip(columns,pd.to_numeric(row.tolist(), errors='ignore'))),ignore_index=True)
                dfo = dfo._append(dict(zip(columns,pd.to_numeric(row.tolist(), errors='ignore'))),ignore_index=True)
